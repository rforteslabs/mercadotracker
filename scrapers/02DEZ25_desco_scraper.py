...
def scrape(driver, url):
    ...
    try:
        ...
        unavailable_element = soup.find(lambda tag: tag.name in ['p', 'div', 'span'] and 'Produto indisponível' in tag.get_text())
        
        if unavailable_element:
            logging.warning(f"Product unavailable at Desco URL: {url}. Skipping.")
            return "UNAVAILABLE"
        else: 
            # Se o produto não estiver indisponível, procura pela classe .vip-card-produto
            # Procura por qualquer ocorrência (mesmo parcial) de "indispon" no texto das tags ou em quaisquer atributos
            vip_card_element = soup.find_all(
                lambda tag: bool(re.search(r"indispon", (tag.get_text() or ""), re.I))
                or any(re.search(r"indispon", str(v), re.I) for v in tag.attrs.values())
            )
            # print(f'.vip-card-produto-indisponivel={vip_card_element}')
